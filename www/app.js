...
function loadMap() {
    ...
    lyrCtrl.addBaseLayer(gsat, "แผนที่ภาพถ่าย (google)");
    ...
    lyrCtrl.addOverlay(pk_parcel, pk_parcel_legend + " แปลงที่ดิน");
    lyrCtrl.addOverlay(pk_green_09082020.addTo(map), "พื้นที่สีเขียว<br>" + pk_green_09082020_legend);
    lyrCtrl.addOverlay(pk_os.addTo(map), pk_os_legend + "พื้นที่เปิดโล่ง<br>");
    lyrCtrl.addOverlay(pk_streams.addTo(map), pk_streams_legend + "แม่น้ำ<br>");
    lyrCtrl.addOverlay(pk_community.addTo(map), pk_community_legend + "ขอบเขตชุมชน<br>");
...
function updateArea(obj) {
    ...
    $('#count2').html('พื้นที่สีเขียวจำนวน ' + obj.length + ' แห่ง รวม <span class="f-orange">' +
        (sumArea).toLocaleString(undefined, { maximumFractionDigits: 2 }) + '</span> ตร.ม.');
    ...
    console.log(sumArea)
    if (sumArea > 0) {
        $('#tmpDown').html('<h4><span class="badge badge-success">อุณหภูมิของพื้นที่สีเขียวใหม่ ' + tmpGreen.toFixed(2) + '&#8451;</span></h4>');
        $('#tmpDiff').html('<h4><span class="badge badge-warning">ผลต่างของอุณหภูมิ ' + tmpDiff.toFixed(2) + '&#8451;</span></h4>');
        $('#tmpDown2').html('<h4><span class="badge badge-success">อุณหภูมิของพื้นที่สีเขียวใหม่ ' + tmpGreen.toFixed(2) + '&#8451;</span></h4>');
        $('#tmpDiff2').html('<h4><span class="badge badge-warning">ผลต่างของอุณหภูมิ ' + tmpDiff.toFixed(2) + '&#8451;</span></h4>');
        $("#PopupG").modal()
    } else {
        $('#tmpDown').empty();
        $('#tmpDiff').empty();
